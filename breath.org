...
This module requires a 12V, minimum 2A brick power supply with a centre positive 2.1mm DC jack plug (eg. MEAN WELL GST40A12 Desktop power
supply). This is not included.
...
price: was 35 costs each say 145 each module 110x50edition=5500 = 12k both // say 165 with 60 costs

ERD/LICHT transforms and amplifies the oscillations and perturbations
of photons into the massed movements of electrons, thus electricity
and perhaps opnly then into sound waves. There is one amplified output
(range?)  and one gate output (2ms 6v - test?), corresponding to the
rising edge of a dazzling approach (a disco strobe for
example). Events can be triggered by excessive photonic eruptions or
avalanche. Both are under the control of one amplitude knob - in most
cases, the gate will need this adjustment.

label: out, gate out, amplitude, light sensor.


output voltage 18Vpp

gate voltage is 0-5V 2mS

power +12v=13mA

-12v=13mA

trigger threshold should be +2v? -> reduce to 1v with R2 as 1K?
